...
namespace RHVoice
{
  ...
  smart_ptr<language> kazah_info::create_instance() const
  {
    return smart_ptr<language>(new kazah(*this));
  }

  kazah::kazah(const kazah_info& info_):
    language(info_),
    info(info_),
...
g2p_p_fst(path::join(info_.get_data_path(),"g2p_p.fst")),
  ...
  std::vector<std::string> kazah::get_word_transcription(const item& word) const
  {
